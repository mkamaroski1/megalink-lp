export default function ThirdFeature() {
  return (
    <>
      <div className="bg-thirdbg bg-cover bg-no-repeat text-white min-w-screen min-h-screen border-b-black">
        <div className="flex flex-col items-center justify-center p-16 lg:mx-5">
          <h2 className="lg:text-4xl text-sm font-bold font-poppins text-center pt-14 lg:p-20">
            O QUE É LINK DEDICADO?
          </h2>
          <p className="lg:text-2xl font-semibold font-telegraf text-start lg:px-10 lg:pb-20 hidden lg:flex">
            Sabe aquele Wi-Fi exclusivo que você deseja ter só para você?
            <br /> Bem, um link dedicado é basicamente isso, só que em uma
            escala maior e mais<br/> profissional. Em vez de compartilhar sua conexão
            com outros, um link dedicado é<br/> como ter uma estrada particular só
            para você trafegar na internet. Você obtém<br/> velocidades consistentes
            e confiáveis, sem as oscilações que podem ocorrer em<br/> conexões
            compartilhadas. Então, se você precisa de uma conexão confiável para<br/>
            manter seus negócios fluindo sem problemas, um link dedicado é a
            escolha certa!
          </p>
          <p className="text-sm text-center font-medium font-telegraf py-10 pb-20 flex lg:hidden">
            Sabe aquele Wi-Fi exclusivo que você deseja ter só para você? Bem,
            um link dedicado é basicamente isso, só que em uma escala maior e
            mais profissional. Em vez de compartilhar sua conexão com outros, um
            link dedicado é como ter uma estrada particular só para você
            trafegar na internet. Você obtém velocidades consistentes e
            confiáveis, sem as oscilações que podem ocorrer em conexões
            compartilhadas. Então, se você precisa de uma conexão confiável para
            manter seus negócios fluindo sem problemas, um link dedicado é a
            escolha certa!
          </p>
        </div>
      </div>
    </>
  );
}
